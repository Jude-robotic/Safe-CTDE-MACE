...
struct VoxelBox {
  Eigen::Vector3i min_corner;
  Eigen::Vector3i max_corner;
};
...
vector<VoxelBox> loadBoxesFromEpisodeJson(const string& path) {
  vector<VoxelBox> boxes;
  const string text = readTextFile(path);
  if (text.empty()) return boxes;

  regex box_re(
      "\\\"min_corner\\\"\\s*:\\s*\\[\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\]\\s*,\\s*"
      "\\\"max_corner\\\"\\s*:\\s*\\[\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\]");
  auto begin = sregex_iterator(text.begin(), text.end(), box_re);
  auto end = sregex_iterator();
  for (auto iter = begin; iter != end; ++iter) {
    VoxelBox box;
    box.min_corner = Eigen::Vector3i(stoi((*iter)[1].str()), stoi((*iter)[2].str()), stoi((*iter)[3].str()));
    box.max_corner = Eigen::Vector3i(stoi((*iter)[4].str()), stoi((*iter)[5].str()), stoi((*iter)[6].str()));
    boxes.push_back(box);
  }
  return boxes;
}
...
void RandomMapGenerateVoxelBox() {
  pcl::PointXYZ pt_random;
  cloudMap.clear();

  vector<VoxelBox> boxes;
  if (!_episode_json.empty()) {
    boxes = loadBoxesFromEpisodeJson(_episode_json);
    ROS_WARN("Loaded %zu voxel boxes from %s", boxes.size(), _episode_json.c_str());
  }

  if (boxes.empty()) {
    ROS_WARN("No episode JSON boxes found; using qmix_ego_large fallback center box only.");
    boxes.push_back({Eigen::Vector3i(8, 8, 0), Eigen::Vector3i(11, 11, 5)});
  }

  const double point_step = max(min(_resolution, _voxel_resolution), 0.05);
  auto generate_voxel = [&](int ix, int iy, int iz) {
    const double min_x = ix * _voxel_resolution;
    const double min_y = iy * _voxel_resolution;
    const double min_z = iz * _voxel_resolution;
    const double max_x = (ix + 1) * _voxel_resolution;
    const double max_y = (iy + 1) * _voxel_resolution;
    const double max_z = (iz + 1) * _voxel_resolution;
    for (double x = min_x + point_step / 2.0; x < max_x; x += point_step)
      for (double y = min_y + point_step / 2.0; y < max_y; y += point_step)
        for (double z = min_z + point_step / 2.0; z < max_z; z += point_step) {
          if (x < 0.0 || y < 0.0 || z < 0.0 || x > _x_size || y > _y_size || z > _z_size)
            continue;
          pt_random.x = x;
          pt_random.y = y;
          pt_random.z = z;
  ...
  for (const auto& box : boxes) {
    for (int ix = box.min_corner.x(); ix <= box.max_corner.x(); ++ix)
      for (int iy = box.min_corner.y(); iy <= box.max_corner.y(); ++iy)
        for (int iz = box.min_corner.z(); iz <= box.max_corner.z(); ++iz)
          generate_voxel(ix, iy, iz);
  }
  ...
  ROS_WARN("Finished generate Safe-CTDE voxel map with %zu boxes and %zu points",
           boxes.size(), cloudMap.points.size());
...
int main(int argc, char** argv) {
  ...
  n.param("seed", _seed, 7);
  n.param("episode_json", _episode_json, string(""));
  ...
  n.param("map/voxel_resolution", _voxel_resolution, 1.0);
